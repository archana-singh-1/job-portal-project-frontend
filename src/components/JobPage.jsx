...
const JobsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchJobs = async () => {
      ...
      try {
        // Fixed API URL to correctly use query parameters
        const response = await fetch(`https://job-portal-project-theta.vercel.app/api/employer/jobs?category=${categoryParam}`);
        if (!response.ok) {
          throw new Error('Failed to fetch jobs');
        }
        const data = await response.json();
        setJobs(data); // Assuming the API directly returns the array of jobs
      } catch (err) {
        setError('Error fetching jobs: ' + err.message);
      } finally {
        setLoading(false);
